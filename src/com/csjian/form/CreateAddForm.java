...
public class CreateAddForm extends PdfPageEventHelper {
	...
	public void renderForm(String regcode, Vector people, InputStream pdftemplate, CompanyBean company, OutputStream os)
			throws Exception {
		try {
			...
			for (int k = 0; k < pages; k++) {
				...
				PdfReader reader = new PdfReader(new FileInputStream("d:/blankform/addform.pdf"));
				...
				for (int i = 0; i < 10; i++) {
					int idx = k * 10 + i;
					if (idx < count) {
						String[] person = (String[]) people.elementAt(idx); // person
																			// {員工編號,是否眷屬,姓名,身份證號,出生日期,加保金額,稱謂,到職日,員工姓名,員工身份證號,員工出生日期}

						if (person[1].equals("N"))
							form.setField("c2." + i + ".0", "Y");
						else
							form.setField("c2." + i + ".1", "Y");
						...
						if (person[1].equals("N"))
							form.setField("t12." + i, "到職");
						else
							form.setField("t12." + i, "眷屬");
						String[] adate = person[11].split("-");
						// form.setField("t13." + i,
						// (cal.get(Calendar.YEAR)-1911)+"."+(cal.get(Calendar.MONTH)+1)+"."+cal.get(Calendar.DATE));
						form.setField("t13." + i,
								(Integer.parseInt(adate[0]) - 1911) + "." + adate[1] + "." + adate[2]);
						if (person[1].equals("Y")) {
							...
							// form.setField("t13." + i,
							// (Integer.parseInt(onboard[0])-1911)+"."+onboard[1]+"."+onboard[2]);
						}
					}
				}
