...
public class CreateMailForm extends PdfPageEventHelper {
	...
	public void generatePDF(String regcode, String[] tocode, InputStream pdftemplate, CompanyBean company, OutputStream os) throws Exception {
		...
		String path = "c:/temp/" + dateFormat.format(new Date()) + "/";
		Files.createDirectories(Paths.get(path));
		try {
			for (int k = 0; k < tocode.length; k++) {
				pdflist[k] = path + regcode + "_" + tocode[k] + ".pdf";
				//PdfReader reader = new PdfReader(pdftemplate);
				PdfReader reader = new PdfReader(new FileInputStream("d:/blankform/mailform.pdf"));
			...
			document.close();
			/*
			 * for (int i=0; i<pdflist.length-1; i++) { File fout = new File(pdflist[i]);
			 * fout.delete(); }
			 */
		} catch (Exception e) {
			e.printStackTrace();
