...
public class P04Reporter extends PdfPageEventHelper {
  ...
  public static void generatePDF(String regcode, String year, String month, CompanyBean company, List salaryDigest, OutputStream os) throws Exception {
    P04Reporter.year = year;
    P04Reporter.month = month;

	  try {
	  	if (company != null) {
        P04Reporter.companyname = company.getRegname();

        Document doc = new Document(PageSize.A4, 50, 50, 100, 72);
        PdfWriter writer = PdfWriter.getInstance(doc, os);
        writer.setPageEvent(new P04Reporter());
        doc.open();
        ...
        tbl.addCell(new Phrase("簽領", FontChineseS));
        int total = 0;
        for (int i=0; i<salaryDigest.size(); i++) {
        	String[] item = (String[]) salaryDigest.get(i);
        	tbl.getDefaultCell().setHorizontalAlignment(Element.ALIGN_CENTER);
  ...
  public void onOpenDocument(PdfWriter writer, Document document){
      ...
      PdfPCell cell = null;
	    cell = new PdfPCell(new Phrase(P04Reporter.companyname +  (Integer.parseInt(P04Reporter.year)-1911) + "年" + P04Reporter.month + "月薪資印領清冊", FontChineseL));
	    cell.setColspan(2);
	    cell.setHorizontalAlignment(Element.ALIGN_CENTER);
