...
public class EmployeeListPDF extends PdfPageEventHelper {
  ...
  public static void generatePDF(CompanyBean company, List<EmployeeBean> employees, OutputStream os) throws Exception {
	  try {
	  	if (company != null) {
        EmployeeListPDF.companyname = company.getRegname();

        Document doc = new Document(PageSize.A4, 50, 50, 100, 72);
        PdfWriter writer = PdfWriter.getInstance(doc, os);
        writer.setPageEvent(new EmployeeListPDF());
        doc.open();
	    ...
	    float[] widths = {0.12f, 0.12f, 0.12f, 0.12f, 0.37f, 0.15f};
        ...
        tbl.addCell(new Phrase("職稱", FontChineseS));
        tbl.addCell(new Phrase("姓名", FontChineseS));
        tbl.addCell(new Phrase("身份證號", FontChineseS));
        tbl.addCell(new Phrase("住址", FontChineseS));
        tbl.addCell(new Phrase("銀行帳號", FontChineseS));
        EmployeeBean employee = null;
        tbl.getDefaultCell().setHorizontalAlignment(Element.ALIGN_LEFT);
        for (int i=0; i<employees.size(); i++) {
        	employee = employees.get(i);
        	tbl.addCell(new Phrase(employee.getEmployeeno()+"", FontChineseS));
        	tbl.addCell(new Phrase(employee.getTitle(), FontChineseS));
        	tbl.addCell(new Phrase(employee.getName(), FontChineseS));
        	tbl.addCell(new Phrase(employee.getUnicode(), FontChineseS));
        	tbl.addCell(new Phrase(employee.getAddress(), FontChineseS));
        	tbl.addCell(new Phrase(employee.getAccountno(), FontChineseS));
        }
  ...
  public void onOpenDocument(PdfWriter writer, Document document){
      ...
      PdfPCell cell = null;
	  cell = new PdfPCell(new Phrase(EmployeeListPDF.companyname +  " 員工基本資料表", FontChineseL));
	  cell.setColspan(2);
