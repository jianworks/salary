...
<script type="text/javascript">
	...
	$(document).ready(function () {
	    oTable = $("#tblList").dataTable({
	        ...
	        "sAjaxSource": '<%=response.encodeURL("salary.do?action=itemDataTable") %>' + "&itemType=M",
	        "aoColumns": [
	            { "sName": "name" },
	            { "sName": "addOnFeeFlag" },
	            { "sName": "enable" },
	            {
				    ...
				    "fnRender": function (oObj) {
					    return "<input type='button' value='修改' onclick=\"doEdit('" + oObj.aData[3] + "')\" class='ui-state-default' />";
					}
				}
	        ]
	    });
	    ...
	    $("#ItemForm").dialog({
		    ...
		    buttons: {
			    "儲存": function () {
			        $.postJSON('<%=response.encodeURL("salary.do?action=updateItem") %>', { itemType:'M', name: $('#name').val(), seqno: $('#seqno').val(), enable: $('#enable').val(), addOnFeeFlag: $('#addOnFeeFlag').val(), regcode: $('#regcode').val() },
                        function (result) {
						    if (result == 'OK') {
							    alert("儲存薪資減項成功 !");
				...
				close: function () {
					oTable.fnReloadAjax('<%=response.encodeURL("salary.do?action=itemDataTable") %>' + "&itemType=M");
				}
	    });
	});
...
<div class="ui-widget-content ui-corner-all" style="width: 98%; margin: 5px; padding: 2px" align="right">
	<form method="POST" Name="main-search" Action="<%=response.encodeURL("salary.do?action=pitem") %>" class = "entry-form" onSubmit='return validate();'>        
        <input type="button" class='ui-state-default' value="增加薪資減項" onclick="doAdd();" />&nbsp;&nbsp;
	</form>
</div>
...
<div id="ItemForm" title="新增修改薪減項" class="ui-widget-content ui-corner-all entry-form">
    <fieldset>
        <div><label for="name" class="styled">薪資減項名稱</label><input id="name" type="text" class="textfield" size="30" maxlength="30"></div>
        <div><label for="addOnFeeFlag" class="styled">要加回二代健保薪資</label>
    ...
    <div id="sysItem" class="ui-widget-content ui-corner-all ui-state-highlight" style="width: 450px; margin: 5px; padding: 2px"><b>
		說明：<br/>
		此薪資減項為系統預設，無法設定名稱及是否要加回二代健保薪資</b>
	</div>
