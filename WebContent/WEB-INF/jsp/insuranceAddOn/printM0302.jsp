...
<script type="text/javascript">
	...
	$(document).ready(function () {
	    oTable = $("#tblList").dataTable({
	        ...
	        "sAjaxSource": '<%=response.encodeURL("insuranceAddOn.do?action=m0302DataTable") %>' + "&incomePeriod=" + $('#incomePeriod').val() + "&incomeType=" + $('#incomeType').val() + "&unicode=" + $('#unicode').val() + "&name=" + $('#name').val(),
	        "aoColumns": [
	        	{
					"sName": "Blank1",
					"bSearchable": false,
	...
	function doPrint() {
		...
		window.location.href='<%=response.encodeURL("insuranceAddOn.do?action=doPrintM0302") %>' + "&serialNos=" + serialNos;
	}
	
	function doSearch() {
	    oTable.fnReloadAjax('<%=response.encodeURL("insuranceAddOn.do?action=m0302DataTable") %>' + "&incomePeriod=" + $('#incomePeriod').val() + "&incomeType=" + $('#incomeType').val() + "&unicode=" + $('#unicode').val() + "&name=" + $('#name').val());
	}
</script>

<div class="ui-widget-content ui-corner-all" style="width: 98%; margin: 5px; padding: 2px" align="right">
	<form method="POST" Name="main-search" Action="<%=response.encodeURL("insuranceAddOn.do?action=printM0302") %>" class = "entry-form" onSubmit='return validate();'>        
